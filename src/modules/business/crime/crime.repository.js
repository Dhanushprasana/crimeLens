...
module.exports = {
  async addCrime(dto, req) {
    ...
    if (dto.incident_registered_date) {
      crime_date = dto.incident_registered_date.split(' ')[0];
    }
    ...
    if (district_id && police_station_id && crime_category_id) {
      ...
      for (const gender of genders) {
        try {
          const checkQuery = `SELECT ROWID, crime_count FROM ${env.TABLE_COMP_DISTRICT_CRIME_STATS} WHERE district_id = '${district_id}' AND police_station_id = '${police_station_id}' AND crime_category_id = '${crime_category_id}' AND gender = '${gender}' AND crime_date = '${crime_date}'`;
          ...
          if (existing && existing.length > 0) {
            ...
            const newCount = parseInt(statRow.crime_count || statRow['COUNT(i.ROWID)'] || statRow.COUNT || 1, 10) + 1;
            await statsTable.updateRow({ ROWID: statRow.ROWID, crime_count: newCount });
          } else {
            await statsTable.insertRow({ district_id, police_station_id, crime_category_id, gender, crime_date, crime_count: 1 });
          }
        } catch (e) {
