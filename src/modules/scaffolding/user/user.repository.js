...
module.exports = {
  async createUser(dto, req) {
    ...
    // 3. Save profile metadata first in sys_user_info table (to retrieve its ROWID for foreign key user_info_id)
    ...
    const savedUser = await userTable.insertRow({
      ...
      is_archived: false,
      catalyst_user_id: dto.catalystUserId || null
    });
  ...
  async restoreUser(id, req) {
    ...
    throw new Error('Restore not supported: sys_user table does not support soft-deletion columns.');
  },

  async getAllUsersV2(query, req) {
    const usersList = await this.getAllUsers(query, req);
    const enrichedUsers = Array.isArray(usersList) ? usersList : usersList.users;
  ...
  async deactivateUser(email, req) {
    ...
    if (roleIds.length > 0) {
      const roleQuery = `SELECT * FROM ${env.TABLE_ROLE} WHERE ROWID IN (${roleIds.map(id => `'${id}'`).join(',')})`;
      const roleResult = await executeQuery(req, roleQuery);
      isSuperAdmin = roleResult.some(r => r[env.TABLE_ROLE].role_name === SUPER_ADMIN_ROLE);
    }
    ...
    if (isSuperAdmin) {
      ...
      if (activeAdminResult && activeAdminResult.length > 0) {
        ...
        if (totalAdminsResult.length <= 1) {
          throw new Error('Cannot deactivate this user. At least one superadmin must remain active.');
        }
      }
    }
  ...
  async activateUser(email, req) {
    ...
    const user = userResult[0][env.TABLE_USER];
    if (user.is_archived === false || user.is_archived === 'false' || user.is_archived === null) {
